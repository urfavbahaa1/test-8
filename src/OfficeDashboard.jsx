...
// شعار المحل نفسه المستخدم في موقع الزبون — لعرضه بدل حرف "P" حين لا يكون
// هناك logo_url مضبوط بعد في إعدادات الوكالة.
import { LOGO_IMG } from "./lib/brandAssets.js";
// خلفية الإنارة السينمائية المتحركة تلقائيًا (بدون سكرول) — بديل حرف "P"
// الكبير الذي كان خلفية كل صفحات اللوحة.
import { DashboardAmbientLights } from "./CinematicLights.jsx";
...
// حساب دخول صاحب الوكالة. اللوحة تعرض يوزرنيم وباسوورد فقط (لا بريد
// إلكتروني في أي مكان بالواجهة) — الاسم الفعلي يُحفظ في عمود
// agencies.admin_username، ويُحوَّل إلى بريد الدخول الحقيقي عبر دالة
// قاعدة البيانات resolve_admin_email(username) بدل أي تخمين ثابت هنا.
async function usernameToEmail(username) {
  const clean = (username || "").trim();
  const { data: viaAgency } = await supabase.rpc("resolve_admin_email", { p_username: clean });
  if (viaAgency) return viaAgency;
  const { data: viaProfile } = await supabase.rpc("get_login_email", { p_username: clean });
...
// يمنع أي استدعاء لـSupabase من التعليق إلى الأبد (مثلاً إن كان مشروع
// Supabase نائمًا مؤقتًا أو الشبكة بطيئة جدًا) — بعد المهلة المحددة تُرفض
// العملية برسالة واضحة بدل بقاء شاشة التحميل معلّقة بلا نهاية.
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`تعذر الاتصال بالخادم (${label}). تحقق من اتصالك وأعد المحاولة.`)), ms)
    ),
  ]);
}

// ---------------------------------------------------------------------------
// كاش خفيف بذاكرة الصفحة لبيانات لوحة التحكم (السيارات/الحجوزات).
// كل صفحة قسم (Dashboard/Cars/Bookings/Calendar) كانت تجلب بياناتها من
// الصفر عند كل دخول، وتعرض شاشة "جار التحميل" الكاملة في كل مرة حتى لو
// كانت البيانات نفسها التي عُرضت قبل ثوانٍ — وهذا سبب الشعور بالتنقّل
// البطيء/المتقطّع بين الأقسام. الآن: أول تحميل فعلي فقط يعرض شاشة
// التحميل؛ أي عودة لاحقة لنفس القسم تعرض آخر بيانات محفوظة فورًا بدون
// أي شاشة تحميل، وتُحدَّث بصمت في الخلفية في حال تغيّر شيء فعليًا.
const dashCache = { cars: null, bookings: null, agencyId: null };
function getCachedCars(agencyId) {
  return dashCache.agencyId === agencyId ? dashCache.cars : null;
}
function getCachedBookings(agencyId) {
  return dashCache.agencyId === agencyId ? dashCache.bookings : null;
}
function setCachedCars(agencyId, cars) {
  dashCache.agencyId = agencyId;
  dashCache.cars = cars;
}
function setCachedBookings(agencyId, bookings) {
  dashCache.agencyId = agencyId;
  dashCache.bookings = bookings;
}
...
const api = {
  ...
  // اسم المستخدم الحالي المعروض في الإعدادات — من agencies.admin_username
  // مباشرة (نفس العمود الذي تعتمد عليه resolve_admin_email).
  ...
  // تغيير اسم المستخدم و/أو كلمة المرور من داخل لوحة التحكم نفسها. اسم
  // المستخدم يُحدَّث في agencies.admin_username (نفس العمود الذي تقرأ منه
  // resolve_admin_email)، وكلمة المرور عبر حساب Supabase Auth مباشرة.
  async updateCredentials({ newUsername, newPassword }) {
    if (newUsername) {
      const clean = newUsername.trim().toLowerCase();
      const { error: agErr } = await supabase
        .from("agencies")
        .update({ admin_username: clean })
        .eq("id", AGENCY_ID);
      if (agErr) throw new Error("تعذر تحديث اسم المستخدم: " + agErr.message);
    }
    if (newPassword) {
      const { error } = await supabase.auth.updateUser({ password: newPassword });
      if (error) throw new Error("تعذر تحديث كلمة المرور: " + error.message);
    }
  },
    ...
    <div className="login-screen login-screen-solo">
      <form className="login-card" onSubmit={handleSubmit}>
        <div className="login-brand">
          <img src={LOGO_IMG} alt="HOUSSEM" className="brand-mark-logo" />
...
export default function OfficeApp() {
  ...
  const loadContext = useCallback(async () => {
    try {
      const ctx = await withTimeout(api.getMyAgency(), 12000, "بيانات الوكالة");
      setAgencyCtx(ctx);
      setCtxError("");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await withTimeout(api.getSession(), 12000, "الجلسة");
        if (cancelled) return;
        if (s) {
          setSession(s);
          await loadContext();
    ...
    <div className="shell">
      <aside className={`sidebar ${drawerOpen ? "open" : ""}`}>
        <div className="sidebar-brand">
          {agency.logo_url ? (
            ...
            <img src={LOGO_IMG} alt={agency.name} className="brand-mark-logo" />
          )}
      ...
      <div className="shell-main">
        <header className="topbar">
          ...
          <div className="topbar-title">
            {agency.logo_url ? (
              ...
              <img src={LOGO_IMG} alt={agency.name} className="topbar-logo" />
            )}
        ...
        <main className="page-content">
          {/* key={page} يجعل الانتقال بين الصفحات يعيد رسم هذا الجزء من
             الصفر في كل مرة، فتنطلق حركة الظهور المتدرجة (page-fade)
             تلقائيًا في كل تنقل داخل اللوحة دون أي كود إضافي. */}
...
// عدّاد صاعد بسيط لأرقام بطاقات الإحصائيات — يبدأ من 0 ويصل للرقم الحقيقي
// خلال أقل من ثانية عند كل تحميل للوحة، بدون أي مكتبات إضافية.
function CountUp({ value, duration = 800 }) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      // ease-out بسيط حتى لا يبدو العدّ خطيًا وجافًا.
...
function DashboardPage({ t, agencyId, go }) {
  // إن كانت البيانات محفوظة من زيارة سابقة لهذا القسم في نفس الجلسة نعرضها
  // فورًا بدون أي شاشة تحميل، ثم نحدّثها بصمت في الخلفية.
  const cachedCars = getCachedCars(agencyId);
  const cachedBookings = getCachedBookings(agencyId);
  const [cars, setCars] = useState(cachedCars || []);
  const [bookings, setBookings] = useState(cachedBookings || []);
  const [loading, setLoading] = useState(!(cachedCars && cachedBookings));
  ...
  useEffect(() => {
    (async () => {
      ...
      setCars(c);
      setBookings(b);
      setCachedCars(agencyId, c);
      setCachedBookings(agencyId, b);
      setLoading(false);
    })();
  }, [agencyId]);
  ...
  const stats = [
    { icon: Car, label: t.stat_total_cars, value: totalCars, numeric: true },
    { icon: Layers, label: t.stat_available_cars, value: availableCars, numeric: true },
    { icon: ClipboardList, label: t.stat_total_bookings, value: totalBookings, numeric: true },
    { icon: Clock, label: t.stat_new_requests, value: newRequests, numeric: true },
    {
      icon: TrendingUp,
...
function CarsPage({ t, agencyId }) {
  const cachedCars = getCachedCars(agencyId);
  const [cars, setCars] = useState(cachedCars || []);
  const [loading, setLoading] = useState(!cachedCars);
  ...
  // silent=true (تنقّل عادي بين الأقسام مع بيانات محفوظة سابقًا): نحدّث
  // البيانات في الخلفية بدون إظهار شاشة التحميل الكاملة من جديد. غير ذلك
  // (أول دخول، أو بعد حفظ/حذف سيارة) تبقى شاشة التحميل كما كانت.
  const reload = useCallback(async (silent) => {
    if (!silent) setLoading(true);
    const fresh = await api.getAgencyCars(agencyId);
    setCars(fresh);
    setCachedCars(agencyId, fresh);
    setLoading(false);
  }, [agencyId]);

  useEffect(() => {
    reload(Boolean(cachedCars));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [reload]);
  ...
  return (
    <div>
      <div className="page-head">
        <h2>{t.cars_title}</h2>
        <button className="btn add-car-btn" onClick={openAdd}>
          <span className="add-car-edge add-car-edge-top" />
          <span className="add-car-edge add-car-edge-right" />
          <span className="add-car-edge add-car-edge-bottom" />
          <span className="add-car-edge add-car-edge-left" />
          <span className="add-car-btn-content">
...
function BookingsPage({ t, agencyId }) {
  ...
  const [bookings, setBookings] = useState(cachedBookings || []);
  const [cars, setCars] = useState(cachedCars || []);
  const [loading, setLoading] = useState(!(cachedBookings && cachedCars));
  ...
  useEffect(() => {
    // بيانات محفوظة سابقًا → تحديث صامت بدون شاشة تحميل كاملة من جديد.
    reload(Boolean(cachedBookings && cachedCars));
...
function CalendarPage({ t, agencyId }) {
  const cachedCars = getCachedCars(agencyId);
  const [cars, setCars] = useState(cachedCars || []);
  const [carId, setCarId] = useState(cachedCars && cachedCars.length ? cachedCars[0].id : "");
  ...
  useEffect(() => {
    (async () => {
      ...
      setCars(list);
      setCachedCars(agencyId, list);
      if (list.length && !carId) setCarId(list[0].id);
      setLoading(false);
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [agencyId]);
...
function CredentialsPanel() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const uname = await api.getCurrentUsername();
      if (!cancelled) setCurrentUsername(uname);
    })();
    return () => {
      cancelled = true;
    };
    ...
    <div className="panel" style={{ marginTop: 20 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {newPassword && newPassword.length < 6 && (
          <p style={{ fontSize: "0.78rem", color: "var(--text-sec)", marginTop: -8 }}>
  ...
  return (
    <style>{`
      ...
      /* ---------------- Dashboard ambient background ----------------
         بديل حرف "P" الكبير السابق: نفس صورة الإنارة السينمائية المستخدمة
         في موقع الزبون، تُشعل تلقائيًا (بدون أي علاقة بالـscroll) فور دخول
         لوحة التحكم ثم يومض الـhazard تلقائيًا بعدها ويبقى خلفية ثابتة
         خلف كل الصفحات. --------------------------------------------- */
      .dashboard-ambient{
        position:fixed; inset:0; overflow:hidden; z-index:-1; pointer-events:none;
        background:#000; opacity:0.4;
      }
      .dashboard-ambient .cl-media{ position:absolute; inset:0; overflow:hidden; }
      .dashboard-ambient .cl-photo{
        position:absolute; inset:-4% -2%; width:104%; height:112%;
        object-fit:cover; object-position:center; filter:brightness(1.04);
      }
      .dashboard-ambient .cl-haze{
        position:absolute; inset:0;
        background:radial-gradient(ellipse at 50% 42%, rgba(255,250,235,0.07), transparent 65%);
        opacity:calc(var(--glow-op, 0) * 0.85);
      }
      .dashboard-ambient .cl-ground-reflection{
        position:absolute; left:6%; right:6%; bottom:0; height:20%;
        background:linear-gradient(to top, rgba(255,244,214,0.32), transparent);
        filter:blur(6px); opacity:calc(var(--glow-op, 0) * 0.55); mix-blend-mode:screen;
      }
      /* نفس تقنية "إشعال واقعي من نفس الصورة" المستخدمة في موقع الزبون:
         رقعة داكنة بشكل كل مصباح تختفي تدريجيًا لتكشف الضوء الحقيقي
         المصوَّر أصلاً مضاءً، بدل دائرة بيضاء/كهرمانية مصطنعة. */
      .dashboard-ambient .cl-headlight-off{
        position:absolute; transform:translate(-50%,-50%); border-radius:50%;
        background:radial-gradient(ellipse at center, #08080a 0%, rgba(8,8,10,0.92) 42%, rgba(8,8,10,0) 76%);
        opacity:var(--dark-op, 1); filter:blur(3px);
      }
      .dashboard-ambient .cl-hazard-off{
        position:absolute; transform:translate(-50%,-50%); border-radius:50%;
        background:radial-gradient(ellipse at center, #060607 0%, rgba(6,6,7,0.95) 45%, rgba(6,6,7,0) 78%);
        opacity:1; filter:blur(2px);
      }
      .dashboard-ambient.hazard-on .cl-hazard-off{
        animation:clHazardOffPulse 2.1s ease-in-out infinite;
      }
      @keyframes clHazardOffPulse{
        0%, 58%, 100%{ opacity:1; }
        10%{ opacity:0.15; }
        22%{ opacity:0; }
        36%{ opacity:0.1; }
      ...
      @media (prefers-reduced-motion: reduce){
        .dashboard-ambient .cl-photo{ transform:none !important; }
        .dashboard-ambient.hazard-on .cl-hazard-off{ animation:none; opacity:0.3; }
      }
      @media (max-width:640px){
        .dashboard-ambient{ opacity:0.26; }
      }
      ...
      /* ---------- LED frame: same "black background + light chasing the
         edges" style used across the customer-facing site's cards and
         "احجز الآن" button — applied here to the dashboard's primary
         buttons too, same gold color, same text, only the fill style and
         a bit of automation (the traveling light) changed. ---------- */
      @keyframes ledChase{ to{ background-position:300% 0; } }
      .btn-primary{
        color:var(--gold-light); font-weight:700; border:1px solid transparent;
        background-image:
          linear-gradient(rgba(10,10,10,0.65), rgba(10,10,10,0.65)),
          linear-gradient(90deg, transparent, var(--gold-light), #FFF6DD, var(--gold-light), transparent, transparent, transparent, transparent);
        background-origin:border-box; background-clip:padding-box, border-box;
        background-size:100% 100%, 300% 100%; background-repeat:no-repeat, repeat-x;
        box-shadow:0 8px 22px rgba(0,0,0,0.35), 0 0 10px rgba(232,200,116,0.2);
        animation:ledChase 4.5s linear infinite;
      }
      .btn-primary:disabled{ background-image:none; background:#2a2a2a; color:#8a8a8a; box-shadow:none; cursor:not-allowed; animation:none; }
      @media (prefers-reduced-motion: reduce){ .btn-primary{ animation:none; } }

      /* ---------- "إضافة سيارة" button: its own distinct template ----------
         Dark fill + four thin LED segments that draw themselves outward
         from the center of each edge the moment the page appears (up,
         right, left, down — one line per side), then settle into a slow
         continuous glow pulse. Text/icon untouched. */
      .add-car-btn{
        position:relative; background:#0a0a0a; color:var(--gold-light);
        border:1px solid rgba(232,200,116,0.22); overflow:visible;
        box-shadow:0 8px 22px rgba(0,0,0,0.35);
      }
      .add-car-btn-content{ position:relative; z-index:1; display:inline-flex; align-items:center; gap:6px; }
      .add-car-edge{ position:absolute; pointer-events:none; z-index:0; background:linear-gradient(90deg, transparent, var(--gold-light), #FFF6DD, var(--gold-light), transparent); border-radius:2px; }
      .add-car-edge-top, .add-car-edge-bottom{ height:2px; left:50%; width:50%; transform:translateX(-50%) scaleX(0); animation:addCarEdgeX .55s ease-out forwards, addCarEdgeGlow 2.4s ease-in-out .55s infinite; }
      .add-car-edge-top{ top:-1px; }
      .add-car-edge-bottom{ bottom:-1px; }
      .add-car-edge-left, .add-car-edge-right{ width:2px; height:50%; top:50%; background:linear-gradient(180deg, transparent, var(--gold-light), #FFF6DD, var(--gold-light), transparent); transform:translateY(-50%) scaleY(0); animation:addCarEdgeY .55s ease-out .12s forwards, addCarEdgeGlow 2.4s ease-in-out .67s infinite; }
      .add-car-edge-left{ left:-1px; }
      .add-car-edge-right{ right:-1px; }
      @keyframes addCarEdgeX{ to{ transform:translateX(-50%) scaleX(2); } }
      @keyframes addCarEdgeY{ to{ transform:translateY(-50%) scaleY(2); } }
      @keyframes addCarEdgeGlow{ 0%,100%{ opacity:0.55; } 50%{ opacity:1; } }
      @media (prefers-reduced-motion: reduce){
        .add-car-edge{ animation:none; opacity:0.8; }
        .add-car-edge-top, .add-car-edge-bottom{ transform:translateX(-50%) scaleX(2); }
        .add-car-edge-left, .add-car-edge-right{ transform:translateY(-50%) scaleY(2); }
      }
      @media (min-width:900px){ .add-car-btn:hover{ transform:translateY(-2px); box-shadow:0 14px 30px rgba(154,163,172,0.4); } }
      ...
      /* ---------- Login ---------- */
      /* الشاشة الآن تعرض بطاقة تسجيل الدخول فقط في المنتصف — بدون لوحة
         "مرحبًا بعودتك" الجانبية التي كانت تظهر على الشاشات الواسعة. */
      .login-screen, .login-screen-solo{ min-height:100vh; display:flex; align-items:center; justify-content:center; gap:0; padding:20px; position:relative; overflow:hidden; }
      .login-screen::after{
      ...
      .brand-mark{ width:34px; height:34px; border-radius:9px; background:linear-gradient(150deg,var(--gold-light),var(--gold)); color:var(--accent-black); font-weight:900; font-family:'Poppins',sans-serif; display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow:0 0 0 1px rgba(232,200,116,0.35), 0 4px 14px rgba(184,145,47,0.35); }
      ...
      /* دخول متدرّج لروابط الشريط الجانبي عند أول تحميل للوحة — لمسة
         احترافية بسيطة بدون أي تأثير على أداء التصفح لاحقًا. */
      @keyframes navLinkIn{ from{ opacity:0; transform:translateX(var(--nav-in-x,8px)); } to{ opacity:1; transform:translateX(0); } }
      [dir="rtl"] .sidebar-nav{ --nav-in-x:-8px; }
      .sidebar-link{ display:flex; align-items:center; gap:11px; padding:11px 12px; border-radius:10px; color:var(--text-sec); background:none; border:none; font-size:0.9rem; text-align:start; cursor:pointer; transition:all .18s ease; position:relative; overflow:hidden; animation:navLinkIn .45s ease both; }
      .sidebar-nav .sidebar-link:nth-child(1){ animation-delay:.03s; }
      .sidebar-nav .sidebar-link:nth-child(2){ animation-delay:.08s; }
      .sidebar-nav .sidebar-link:nth-child(3){ animation-delay:.13s; }
      .sidebar-nav .sidebar-link:nth-child(4){ animation-delay:.18s; }
      .sidebar-nav .sidebar-link:nth-child(5){ animation-delay:.23s; }
      ...
      /* ظهور ناعم لمحتوى الصفحة عند كل تنقل بين أقسام اللوحة. */
      @keyframes pageFadeIn{ from{ opacity:0; transform:translateY(8px); } to{ opacity:1; transform:translateY(0); } }
      .page-fade{ animation:pageFadeIn .35s ease both; }
      @media (prefers-reduced-motion: reduce){ .page-fade{ animation:none; } .sidebar-link{ animation:none; } }
      ...
      @keyframes statCardIn{ from{ opacity:0; transform:translateY(10px) scale(.97); } to{ opacity:1; transform:translateY(0) scale(1); } }
      .stat-card{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:16px; display:flex; align-items:center; gap:12px; transition:transform .25s ease, box-shadow .25s ease, border-color .25s ease; animation:statCardIn .4s ease both; }
      ...
      .stat-grid .stat-card:nth-child(4){ animation-delay:.17s; }
      .stat-grid .stat-card:nth-child(5){ animation-delay:.22s; }
